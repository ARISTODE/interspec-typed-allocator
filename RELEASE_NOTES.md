...
This release packages the Extended-SP3 research prototype through P11. The artifact strengthens InterSpec's pointer-domain policy with trusted allocation metadata so covered trusted uses can require a live tracked allocation, the expected trusted type, and a byte extent contained within that allocation.
...
• P11 a matched three-way complete-rsync performance driver on wasm2c that separates tracking/provenance, final validation, and total Extended-SP3 overhead over an RLBox-only runtime path
...
P11 reuses the complete rsync/popt path to create three matched wasm2c configurations: `rlbox_only`, `tracked_no_check`, and `extended_sp3`. The driver rotates execution order across paired repetitions, emits raw complete-process samples, mechanically decomposes tracking/provenance, validation, and total overhead, and records CPU/kernel/frequency-policy metadata. Hosted CI results are reproducibility references only; publication values are intended to be regenerated on controlled hardware using the same driver and artifact format.
...
See `P6_EVALUATION.md`, `P7A_PROVENANCE.md`, `P7B_NATIVE_INTEGRATION.md`, `P7C_GENERALIZATION.md`, `P8_EVALUATION.md`, `P9A_EVALUATION.md`, `P9B_EVALUATION.md`, `P10_WASM2C_P7C.md`, `P11_EVALUATION.md`, and `REPRODUCIBILITY.md` for the detailed scope, design, evaluation, and reproduction commands.
