...
def parse_runtime(rows):
    ...
    for row in rows:
        ...
        result[key] = total_ns / operations
    return result
...
def collect_rlbox_overhead(rows, pair, populations):
    metrics = {}
    for row in rows:
        key = (row["metric"], int(row["population"]))
        if key in metrics:
    ...
    for population in populations:
        ...
        result.append({
            ...
            "additional_ns_per_op": f"{extended['median'] - baseline['median']:.6f}",
            "extended_over_baseline": f"{extended['median'] / baseline['median']:.6f}",
            "baseline_min_ns_per_op": f"{baseline['min']:.6f}",
            "baseline_max_ns_per_op": f"{baseline['max']:.6f}",
            "extended_min_ns_per_op": f"{extended['min']:.6f}",
            "extended_max_ns_per_op": f"{extended['max']:.6f}",
        })
    return result
...
def main():
    ...
    parser.add_argument("--rlbox-runtime")
    parser.add_argument("--require-boundary-evidence", action="store_true")
    parser.add_argument("--require-rlbox-runtime", action="store_true")
